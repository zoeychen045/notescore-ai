...
def calc_textarea_height(text: str, min_height: int = 90, max_height: int = 420, chars_per_line: int = 32) -> int:
    line_count = text.count("\n") + 1
    estimated_wrap_lines = max(1, math.ceil(len(text) / chars_per_line))
    total_lines = max(line_count, estimated_wrap_lines)
    height = total_lines * 28 + 24
...
if result_bundle:
    ...
    title_label_col, title_btn_col = st.columns([9, 1.2])
    ...
    body_label_col, body_btn_col = st.columns([9, 1.2])
    with body_label_col:
        st.markdown("**改写正文**")
    with body_btn_col:
